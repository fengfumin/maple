...
class Tooldb(metaclass=OrangedbSingle):
    ...
    def save(self, obj):
        ...
        sql = "insert into %s(%s) values(%s)" % (obj.__class__.__name__, columns, fmt)
    ...
    def select_many(self, cls, condition=None, args=None, limit=None):
        ...
        for dic in res:
            ...
            for k, v in dic.items():
                #判断是否为datetime类,是就转成字符串
                if isinstance(v, datetime):
                    v = str(v)
                obj.__dict__[k] = v
            objs.append(obj)
        return objs  # 返回的是列表形式的多字典
...
class Connection:
    ...
    def __init__(self, max_connect=10, retry_time=0.2):
        self.pool = []
        self.retry_time = retry_time  # 数据库建立等待时间
        self.max_connect = max_connect  # 最大链接池数
        # 首次启动创建2个连接
        try:
            for i in range(2):
                # 建立连接
    ...
    def execute(self, sql, args=None, is_select=False):
        ...
        if is_select:
            # 查询走这个
            return cursor.fetchall()  # 获取数据库表内所有信息

        # 增删改数据走这个
        return affect_row  # 返回的是Ture ,False
